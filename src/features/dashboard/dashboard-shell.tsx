  ...
  return (
    <div className="border border-[var(--border-ink)] bg-[var(--surface)] rounded-[var(--radius-medium)] p-4 min-h-24 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="font-data text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">
            {label}
          </p>
          <p className="font-data mt-1.5 text-3xl font-black text-[var(--foreground)]">
            {value}
          </p>
          <p className="mt-1 text-xs font-data text-[var(--text-secondary)]">
            {detail}
          </p>
        </div>
        <div
...
function MonthlyForecastChart() {
  ...
  const monthlyProjection = useMemo(
    () => buildMonthlyProjection(resources),
    [resources],
  );
  ...
  return (
    <div className="border border-[var(--border-ink)] bg-[var(--surface)] rounded-[var(--radius-medium)] p-4 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-display text-lg font-bold ink">
            Monthly Forecast
          </h3>
          <p className="font-data text-xs text-secondary">
            6-month resource projection for Black Daruma, Jade, & G6
          </p>
        </div>
        <div className="hidden sm:flex items-center gap-4 text-xs font-data font-bold text-[var(--text-secondary)]">
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 bg-[var(--foreground)] border border-[var(--border-ink)] inline-block"></span>{" "}
            BD
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 bg-[var(--text-secondary)] border border-[var(--border-ink)] inline-block"></span>{" "}
            Jade
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 bg-[var(--text-secondary)] border border-[var(--border-ink)] inline-block"></span>{" "}
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <XAxis
              ...
              tickLine={false}
              axisLine={false}
              tick={{ fontSize: 12, fill: "var(--color-ink)", fontWeight: 600 }}
            />
            <YAxis
              tickLine={false}
              axisLine={false}
...
function UpcomingProjectsCard() {
  ...
  const ranked = useMemo(
    () => rankProjects(projects, resources),
    [projects, resources],
  );
  ...
  return (
    <div className="border border-[var(--border-ink)] bg-[var(--surface)] rounded-[var(--radius-medium)] p-4 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h3 className="font-display text-lg font-bold ink">
            Upcoming Build Priorities
          </h3>
          <p className="font-data text-xs text-secondary">
            Ranked by ROI and current resource allocation
          </p>
        </div>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        {ranked.slice(0, 4).map((forecast, index) => {
          ...
          const {
            calculateBlackDarumaCost,
          } = require("@/domain/skill-progression");
          ...
          return (
            <div
              key={project.id}
              className="border border-[var(--border-ink)] bg-[var(--surface)] p-3 shadow-sm rounded-[var(--radius-medium)]"
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="font-data text-xs text-secondary mt-0.5">
                    Grade: {project.unitProgression.gradeProgress.currentGrade}{" "}
                    → {project.unitProgression.gradeProgress.targetGrade} · BD
                    Required: {bdRequired}
                  </p>
                </div>
                <Badge
...
function SummonGoalsCard() {
  ...
  const resources = useInventoryStore((state) => state.resources);
  const plannedThroughputs = require("@/store/activity-store").useActivityStore(
    (state: any) => state.plannedThroughputs,
  );
  ...
  if (!activeGoal) {
    return (
      <div className="border border-[var(--border-ink)] bg-[var(--surface)] rounded-[var(--radius-medium)] p-4 shadow-sm flex flex-col items-center justify-center h-full text-center">
        ...
        <h3 className="font-display text-sm font-bold ink">
          No Active Summon Goal
        </h3>
        <p className="font-data text-xs text-secondary mb-3 mt-1">
          Set a banner target to track savings trajectory.
        </p>
  ...
  const currentJade = resources.jade
    ? resources.jade.currentAmount + (resources.jade.manualAdjustment ?? 0)
    : 0;
  const currentAmulets = resources.mysteryAmulet
    ? resources.mysteryAmulet.currentAmount +
      (resources.mysteryAmulet.manualAdjustment ?? 0)
    : 0;
  ...
  const projectedPulls =
    Math.floor(projectedJade / 100) + Math.floor(projectedAmulets);
    ...
    <div className="border border-[var(--border-ink)] bg-[var(--surface)] rounded-[var(--radius-medium)] p-4 shadow-sm h-full flex flex-col">
      ...
      <div className="flex-1 flex flex-col justify-center">
        <div className="flex items-end justify-between mb-2">
          <div>
            <p className="font-data text-3xl font-black text-[var(--foreground)]">
              {projectedPulls}
            </p>
          ...
          <div className="text-right">
            <p className="font-data text-xl font-bold text-[var(--text-secondary)]">
              / {activeGoal.requiredPulls}
            </p>
        ...
        <Progress
          value={Math.min(
            (projectedPulls / activeGoal.requiredPulls) * 100,
            100,
          )}
          className="h-2 mb-4"
        />
        ...
        <div className="grid grid-cols-2 gap-2 text-xs font-data">
          <div className="bg-[var(--surface)] p-2 rounded border border-[var(--border-ink)]">
            <span className="block text-[var(--text-secondary)] mb-0.5">
              Time Left
            </span>
...
export function DashboardOverviewPage() {
  ...
  const summary = useMemo(
    () => summarizeAccount(resources, projects),
    [projects, resources],
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link
          href="/changelog"
          className="group flex flex-col sm:items-end p-3 border border-accent-gold/30 bg-accent-gold/5 hover:bg-accent-gold/10 transition-colors"
        >
